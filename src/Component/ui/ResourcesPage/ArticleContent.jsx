...
import { facebookIcon, instaIcon, userProfilePic, verifiedBadge } from "@/assets";

const ArticleContent = ({ article, onBack }) => {
  return (
    <>
      <article className="mb-8 lg:w-[90%] xl:w-4/5">
        <img src={article.articleImg} alt="" className="h- " />

        <header>
          <h1 className="text-white text-2xl lg:text-4xl xl:text-5xl font-medium leading-[1.3] md:leading-[1.1] mt-5 ">
            {article.title}
          </h1>
          ...
          <div className="article-meta text-[#d9d9d9] text-sm xl:text-base flex flex-col md:flex-row md:items-center justify-between gap-3 mt-5 tracking-wide ">
            <div className="author-cont flex items-center gap-2 ">
              ...
              <p className="article-author">
                By <span className="text-serene font-medium">{article.author}</span>
              </p>
            </div>

            <p className="article-postDate">Published on {article.date}</p>
          </div>
        </header>
        ...
        <button onClick={onBack} className="text-serene mt-4">
          Back to Articles
        </button>
      </article>
    </>
